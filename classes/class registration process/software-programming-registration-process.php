...
if(isset($_POST['submit'])){
    ...
    $sql = "SELECT COUNT(*) AS user_count FROM spring_software_programming";
    ...
    if ($result) {
        ...
        if ($user_count >= 10) {
            ...
            $sql = "INSERT INTO spring_software_programming (username, firstname, lastname, email) VALUES ('$username', '$firstname', '$lastname', '$email')";
            $result = $conn->query($sql);
            
            if($result){
				echo "<script>
                    alert('You have successfully registered for the Spring Software Programming Course!');
