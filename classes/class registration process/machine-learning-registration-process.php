...
if(isset($_POST['submit'])){
    ...
    $sql = "SELECT COUNT(*) AS user_count FROM fall_machine_learning";
    ...
    if ($result) {
        ...
        if ($user_count >= 10) {
            ...
            $sql = "INSERT INTO fall_machine_learning (username, firstname, lastname, email) VALUES ('$username', '$firstname', '$lastname', '$email')";
            $result = $conn->query($sql);
            
            if($result){
				echo "<script>
                    alert('You have successfully registered for the Fall Machine Learning Course!');
