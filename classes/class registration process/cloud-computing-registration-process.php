...
if(isset($_POST['submit'])){
    ...
    $sql = "SELECT COUNT(*) AS user_count FROM summer_cloud_computing";
    ...
    if ($result) {
        ...
        if ($user_count >= 10) {
            ...
            $sql = "INSERT INTO summer_cloud_computing (username, firstname, lastname, email) VALUES ('$username', '$firstname', '$lastname', '$email')";
            $result = $conn->query($sql);
            
            if($result){
				echo "<script>
                    alert('You have successfully registered for the Summer Cloud Computing Course!');
