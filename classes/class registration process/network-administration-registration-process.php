...
if(isset($_POST['submit'])){
    ...
    $sql = "SELECT COUNT(*) AS user_count FROM summer_network_administration";
    ...
    if ($result) {
        ...
        if ($user_count >= 10) {
            ...
            $sql = "INSERT INTO summer_network_administration (username, firstname, lastname, email) VALUES ('$username', '$firstname', '$lastname', '$email')";
            $result = $conn->query($sql);
            
            if($result){
				echo "<script>
                    alert('You have successfully registered for the Summer Network Administration Course!');
